    ...
    <div class="px-40 flex flex-1 justify-center py-5">
        <div class="layout-content-container flex flex-col max-w-[960px] flex-1">
            <div class="flex flex-wrap justify-between gap-3 p-4">
                <p class="text-[#0d141c] tracking-light text-[32px] font-bold leading-tight min-w-72">Entdecken Sie unsere Produkte
                </p>
            </div>
            <div class="pb-3">
                            ...
                            <div class="product-actions">
                                <div>
                                    ...
                                    <form action="{{ route('cart.add', $item->id) }}" method="POST" class="inline-block">
                                        @csrf
                                        <button type="submit"
                                            class="cart-btn px-9 bg-black text-white rounded hover:bg-gray-800 transition">
                                            In den Warenkorb
                                        </button>
                                    </form>
                                    ...
                                    <div class="flex gap-2 mt-2">
                                        ...
                                        @can('delete', $item)
                                            <form action="{{ route('products.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Wirklich löschen?');"
                                                class="inline-block">
                                                @csrf
                                                @method('DELETE')
